...
function makeRounding(randomInt, choice) {
  ...
  return { template: `${value.toLocaleString("ja-JP")}を${place.label}までのがい数にすると {0}`, answers: [roundTo(value, place.value).toLocaleString("ja-JP")] };
}
...
function makeEstimateAddSub(randomInt, choice) {
  ...
  return { template: `${a.toLocaleString("ja-JP")} ${op} ${b.toLocaleString("ja-JP")}を${place.label}までのがい数で見積もると {0}`, answers: [answer.toLocaleString("ja-JP")] };
}
...
function makeEstimateMultiply(randomInt) {
  ...
  return { template: `${a} × ${b}を百の位・十の位のがい数で見積もると {0}`, answers: [answer.toLocaleString("ja-JP")] };
}

window.WORKSHEET_APP = {
  id: "rounding-print-grade4", stateVersion: 2, title: "4年生 がい数・四捨五入", defaultType: "mixed", defaultCount: 24, defaultColumns: 2, countMax: 60,
